...
def is_question_distinct(new_question: str, existing_questions: List[str]) -> bool:
    ...
    return all(new_question_normalized != q.strip().lower() for q in existing_questions)
...
def check_answer_precision(context: str, questions: List[str], original_answer: str) -> Tuple[List[float], List[str]]:
    generated_answers = [generate_answer(context, question) for question in questions]
    
    try:
        response = client.chat.completions.create(
            model="gpt-4o-2024-08-06",
            messages=[
                {"role": "system", "content": "You are an expert in evaluating answer precision for multiple questions simultaneously."},
                {"role": "user", "content": f"""Given the context, a list of questions, the original answer, and a list of generated answers, evaluate how close each generated answer is to the original answer. Provide a precision score from 0 to 1 for each answer, where 1 means the answers are identical in meaning and 0 means they are completely unrelated.
...
[
  {{
    "question": "What type of performances were prohibited, leading to the decline of medieval drama?",
    "generated_answer": "Religious plays were banned, leading to the decline of medieval drama."
  }},
  {{
    "question": "What did Elizabeth I forbid in 1558?",
    "generated_answer": "Elizabeth I forbade all religious plays in 1558."
  }},
  {{
    "question": "What form of entertainment was suppressed across various European countries in the 16th century?",
    "generated_answer": "Religious dramas and performances were suppressed across various European countries in the 16th century."
  }}
]

Expected output:
{{
  "precision_scores": [0.9, 1.0, 0.8]
}}
...
Questions and Generated Answers:
{json.dumps([{{"question": q, "generated_answer": a}} for q, a in zip(questions, generated_answers)])}

Provide only the precision scores as a list of numbers between 0 and 1, corresponding to each question-answer pair."""}
            ],
            response_format={
                "type": "json_schema",
                "json_schema": {
                    "name": "batch_answer_precision_evaluator",
                    "strict": True,
                    "schema": {
                        "type": "object",
                        "properties": {
        ...
        json_response = response.choices[0].message.content
        parsed_response = json.loads(json_response)
        precision_scores = parsed_response["precision_scores"]
    except Exception as e:
        logger.error(f"Error in evaluating answer precision: {e}")
        precision_scores = [0.5] * len(questions)  # Neutral scores in case of error
