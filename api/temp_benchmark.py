...
def run_benchmark(args):
    ...
    temperatures = [round(t * 0.1, 1) for t in range(0, 11)]  # Temperatures from 0.0 to 1.0 in steps of 0.1
    ...
    for temp in temperatures:
        ...
        print(f"\nRunning benchmark with judge temperature: {judge_temperature}")

        # Run the process on 50 samples with seed 42
        args.num_entries = 25 # was 50
        args.random_seed = 42
        args.entries_file = f"benchmark_results_temp_{temp}.csv"  # Optional: save results per temperature
        ...
        results = []  # To store results in memory

        for idx_in_entries, (entry, idx_in_dataset) in enumerate(
            tqdm(zip(entries_to_process, indices), total=len(indices), desc=f'Processing entries at temperature {temp}')
        ):
            # print(f"Processing entry {idx_in_entries + 1}/{len(indices)} (Dataset Index: {idx_in_dataset})...")
            try:
                result = process_entry(entry)
                result['Index'] = idx_in_dataset  # Add the index to the result
        ...
        df.to_csv(f'benchmark_results_temp_{temp}.csv', index=False)
