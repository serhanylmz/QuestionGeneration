...
PROMPT_TEMPLATE_1 = '''You are an expert judge evaluating the quality of paraphrased questions based on a given context and original question-answer pair. Evaluate their quality and relevance.
...
PROMPT_TEMPLATE_2 = '''Please act as an impartial judge and evaluate the quality of the responses provided by two AI assistants to the user question displayed below. You should choose the assistant that follows the user's instructions and answers the user's question better. Your evaluation should consider factors such as the helpfulness, relevance, accuracy, depth, creativity, and level of detail of their responses. Begin your evaluation by comparing the two responses and provide a short explanation. Avoid any position biases and ensure that the order in which the responses were presented does not influence your decision. Do not allow the length of the responses to influence your evaluation. Do not favor certain names of the assistants. Be as objective as possible. After providing your explanation, output your final verdict by strictly following this format: "Question 1" if Question 1 is better, "Question 2" if Question 2 is better, and "Draw" for a tie.
...
PROMPT_TEMPLATE_3 = '''You will be given two question-answer pairs generated as paraphrases of an original question-answer pair, along with their context. Your task is to evaluate and compare their quality using a structured assessment process.

Please make sure you read and understand these instructions carefully.

Evaluation Criteria:
1. Structural Difference (1-5)
- How well the question uses different wording and structure from the original
- 1: Nearly identical structure
- 3: Moderate structural changes
- 5: Completely different structure while maintaining intent

2. Semantic Preservation (1-5) 
- How well the question maintains the original meaning despite structural changes
- 1: Different meaning/intent
- 3: Partially preserved meaning
- 5: Fully preserved meaning

3. Answer Alignment (1-5)
- How well the answer captures the key information from the original answer
- 1: Missing key information
- 3: Partial information overlap
- 5: Complete information preservation

Evaluation Steps:
1. Read the context, original Q&A pair, and both generated Q&A pairs carefully
2. For each generated Q&A pair:
   - Analyze structural differences from original question
   - Evaluate semantic preservation of original meaning
   - Compare answer alignment with original answer
3. Calculate final scores (sum of all three criteria, max 15 points converted to 1-10 scale)
4. Determine winner based on total scores
...
After completing your evaluation, provide your response in the following JSON format:

"question1_score": <number>, // 1-10 scale, converted from total criteria scores
"question2_score": <number>, // 1-10 scale, converted from total criteria scores
"explanation": "<detailed comparison highlighting key differences and rationale for scores>",
"winner": "<Question 1 or Question 2>" // Based on higher total score

Your response must be a valid JSON object following this exact template. You must pick a winner; it cannot be a draw. Round scores to nearest decimal point.
'''

# Collect the prompts into a list
prompts = [PROMPT_TEMPLATE_1, PROMPT_TEMPLATE_2, PROMPT_TEMPLATE_3]
...
def compare_questions_claude(context: str, original_question: str, original_answer: str,
    ...
    try:
        ...
        response_data = response.content[0].input
...
def run_benchmark(args):
    ...
    # Step 2: For each prompt, evaluate the judges
    for idx, prompt_template in enumerate(prompts, 1):
        prompt_name = f'prompt_{idx}'

        # Check if the Excel file for this prompt already exists
        filename = f'benchmark_results_{prompt_name}.xlsx'
        if args.reverse:
            filename = f'benchmark_results_{prompt_name}_reverse.xlsx'
        if os.path.exists(filename):
            print(f"Results for prompt '{prompt_name}' already exist in '{filename}'. Skipping processing for this prompt.")
        ...
        print(f"\nEvaluating judges with prompt: {prompt_name}")

        # Initialize total vote counts
        total_vote_counts = Counter()
        results_with_verdicts = []

        for result in tqdm(processed_entries, desc=f'Evaluating with {prompt_name}'):
        ...
        # After processing all entries for this prompt, calculate percentage of enhanced generation chosen
        ...
        print(f"Results for prompt '{prompt_name}' saved to '{filename}'.")

    # Now, create a chart/graph showing percentage of enhanced generation chosen per prompt
    prompt_names = [f'prompt_{i+1}' for i in range(len(prompts))]
    plt.figure(figsize=(10, 6))
    plt.bar(prompt_names, percentages)
    plt.title('Percentage of Enhanced Generation Chosen per Prompt')
    plt.xlabel('Prompts')
    ...
    if args.reverse:
        plt.savefig('benchmark_prompt_study_reverse.png')
        print("Benchmark chart saved as 'benchmark_prompt_study_reverse.png'.")
    else:
        plt.savefig('benchmark_prompt_study.png')
        print("Benchmark chart saved as 'benchmark_prompt_study.png'.")
